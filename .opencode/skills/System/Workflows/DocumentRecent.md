...
cat ~/.opencode/MEMORY/PAISYSTEMUPDATES/index.json | jq '.updates[0].timestamp'
```

Or parse from the most recent file:
```bash
ls -t ~/.opencode/MEMORY/PAISYSTEMUPDATES/*/\*/*.md | head -1
```
...
}' | bun ~/.opencode/skills/System/Tools/CreateUpdate.ts --stdin
```
