...
OpenCode supports the following hook events (from `~/.opencode/hooks/lib/observability.ts`):
...
- `CheckVersion.hook.ts` - Checks OpenCode version and notifies of updates
...
- After receiving Task result, parent calls: `bun ~/.opencode/skills/PAI/Tools/UpdateTabTitle.ts "Message"`
- This avoids transcript parsing race conditions
- See `~/.opencode/skills/PAI/Tools/UpdateTabTitle.ts` for implementation
...
**Full Documentation:** See `~/.opencode/skills/PAI/SYSTEM/TERMINALTABS.md`
...
**Root Cause:**
- OpenCode event trigger issue (external to hook system)
...
1. **Added CAPTURE field to response format** (see `~/.opencode/skills/PAI/SKILL.md`)
...
**Status:** UNRESOLVED (OpenCode issue, not hook configuration)
**Mitigation:** Structural enforcement via response format (cannot complete valuable work without verification)
**Tracking:** Documented in `~/.opencode/skills/PAI/SKILL.md` (History Capture System section)

**Long-term Fix:**
- Report to Anthropic (OpenCode team) as Stop event reliability issue
- Monitor future OpenCode updates for fix
...
grep '"type":"user"' ~/.opencode/projects/-Users-daniel--opencode/*.jsonl | head -1 | jq '.type'
...
**Check:**
1. Does `~/.opencode/skills/PAI/SKILL.md` exist?
...
- **Agent System:** `~/.opencode/skills/PAI/SYSTEM/AGENTPERSONALITIES.md`
- **History/Memory:** `~/.opencode/skills/PAI/SYSTEM/MEMORYSYSTEM.md`
...
Path: ~/.opencode/skills/PAI/Tools/Inference.ts
