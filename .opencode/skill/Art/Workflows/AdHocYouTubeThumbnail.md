...
```bash
bun run ~/.opencode/skill/Art/Tools/Generate.ts \
...
# Default location: ~/.opencode/skill/CORE/USER/SKILLCUSTOMIZATIONS/Art/References/
bun ~/.opencode/skill/Headshot/Tools/Headshot.ts \
  --prompt "[FACE-ONLY HEADSHOT PROMPT]" \
  --reference ~/.opencode/skill/CORE/USER/SKILLCUSTOMIZATIONS/Art/References/reference.png \
  --size 2K \
  --aspect-ratio 1:1 \
...
```bash
bun ~/.opencode/skill/CORE/Tools/RemoveBg.ts ~/Downloads/yt-headshot-${TIMESTAMP}.png
```
...
```bash
bun ~/.opencode/skill/Art/Tools/ComposeThumbnail.ts \
