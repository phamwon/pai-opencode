...
```bash
bun ~/.opencode/skill/PAIUpgrade/Tools/Anthropic.ts
```
...
```bash
bun ~/.opencode/skill/CORE/Tools/LoadSkillConfig.ts ~/.opencode/skill/PAIUpgrade youtube-channels.json
```
...
```bash
cat ~/.opencode/skill/PAIUpgrade/State/youtube-videos.json
```

**For new videos, extract transcripts:**
```bash
bun ~/.opencode/skill/CORE/Tools/GetTranscript.ts "<video-url>"
```
...
- User: `~/.opencode/skill/CORE/USER/SKILLCUSTOMIZATIONS/PAIUpgrade/youtube-channels.json`
