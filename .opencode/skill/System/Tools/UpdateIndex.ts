...
function generateRecent(updates: UpdateRecord[], count: number = 10): string {
  ...
  const lines: string[] = [
    ...
    "- `~/.opencode/MEMORY/PAISYSTEMUPDATES/CHANGELOG.md`",
    "- `~/.opencode/MEMORY/PAISYSTEMUPDATES/index.json`",
    "",
    "---",
    "",
  ...
  for (const update of recent) {
    const fullPath = `~/.opencode/MEMORY/PAISYSTEMUPDATES/${update.file_path}`;
