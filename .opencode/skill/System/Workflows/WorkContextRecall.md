...
|----------|-------------------|-----------------|
| **MEMORY/WORK/** | Directory names, META.yaml files | `ls ~/.opencode/MEMORY/WORK/ \| grep -i [keyword]` |
| **MEMORY/STATE/progress/** | Active multi-session projects | `cat ~/.opencode/MEMORY/STATE/progress/*.json \| grep -i [keyword]` |
| **MEMORY/LEARNING/** | Related learnings and patterns | `grep -r [keyword] ~/.opencode/MEMORY/LEARNING/` |
| **MEMORY/RESEARCH/** | Agent output captures | `grep -r [keyword] ~/.opencode/MEMORY/RESEARCH/` |
| **projects/-Users-daniel--claude/** | Session transcripts (last 30 days) | `grep -l [keyword] ~/.opencode/projects/-Users-daniel--claude/*.jsonl` |
| **Plans/** | Planning documents | `grep -r [keyword] ~/.opencode/Plans/` |
| **WORK/** | Root work scratch files | `ls ~/.opencode/WORK/` |
...
ls -lt ~/.opencode/MEMORY/WORK/ | grep -i "[keyword]"

# Read META.yaml for context
cat ~/.opencode/MEMORY/WORK/*/META.yaml | grep -A5 -B5 "[keyword]"
```

### For Session Transcripts
```bash
# Find sessions mentioning the topic (recent first)
ls -t ~/.opencode/projects/-Users-daniel--claude/*.jsonl | head -20 | xargs -I{} sh -c 'grep -l "[keyword]" "{}" 2>/dev/null && echo {}'
...
cat ~/.opencode/MEMORY/STATE/progress/*.json | jq '.name, .objectives, .lastHandoff'
```
