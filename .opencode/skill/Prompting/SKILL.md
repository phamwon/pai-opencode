...
`~/.opencode/skill/CORE/USER/SKILLCUSTOMIZATIONS/Prompting/`
...
```bash
bun run ~/.opencode/skill/Prompting/Tools/RenderTemplate.ts \
...
```bash
bun run ~/.opencode/skill/Prompting/Tools/ValidateTemplate.ts \
  --template Primitives/Briefing.hbs \
...
import { renderTemplate } from '~/.opencode/skill/Prompting/Tools/RenderTemplate.ts';
