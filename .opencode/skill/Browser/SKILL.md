...
`~/.opencode/skill/CORE/USER/SKILLCUSTOMIZATIONS/Browser/`
...
**Headless by default.** All automation runs in headless mode. When the user says "show me" or wants to see what the assistant is seeing, open the URL in their preferred browser from `~/.opencode/skill/CORE/USER/TECHSTACKPREFERENCES.md`:
...
bun run ~/.opencode/skill/Browser/Tools/Browse.ts https://example.com
...
**Location:** `~/.opencode/skill/Browser/Tools/Browse.ts`
...
**Location:** `~/.opencode/skill/Browser/Tools/BrowserSession.ts`
...
```typescript
import { PlaywrightBrowser } from '~/.opencode/skill/Browser/index.ts'
